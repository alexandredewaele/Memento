      ...
      <div className="w-96 shrink-0 border-r border-slate-200 dark:border-slate-800 flex flex-col h-full overflow-y-auto">
        ...
        <div className="px-6 pt-8 pb-4 grid grid-cols-2 gap-3">
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-100 dark:border-slate-700">
            <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider mb-1">Streak</p>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-100 dark:border-slate-700">
            <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider mb-1">Total</p>
            <div className="flex items-end gap-1">
              <span className="text-3xl font-bold text-emerald-400">{totalEntries}</span>
            ...
            {selectedEntry ? (
              <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-sm border border-slate-100 dark:border-slate-700 max-w-2xl">
                <div className="flex items-start justify-between mb-4">
                  <span className="text-xs font-bold uppercase tracking-wider text-slate-400">{selectedEntry.category}</span>
                  <span className={`material-icons-round text-xl ${selectedEntry.is_favorite ? 'text-red-400' : 'text-slate-300 dark:text-slate-600'}`}>
                    {selectedEntry.is_favorite ? 'favorite' : 'favorite_border'}
                  </span>
                </div>
                <h3 className="text-3xl font-bold text-slate-900 dark:text-white mb-4">{selectedEntry.title}</h3>
                <p className="text-base text-slate-600 dark:text-slate-300 leading-relaxed">{selectedEntry.content}</p>
                {selectedEntry.example && (
                  <div className="mt-6 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border-l-4 border-primary">
                    <p className="text-sm text-slate-500 dark:text-slate-400 italic">"{selectedEntry.example}"</p>
                  </div>
                )}
              </div>
            ) : (
              <div className="flex flex-col items-center py-24 text-center gap-3">
                <span className="material-icons-round text-5xl text-slate-300 dark:text-slate-600">event_busy</span>
                <p className="text-slate-500 dark:text-slate-400">No entry recorded for this day.</p>
              </div>
            )}
          </>
        ) : (
          <div className="flex flex-col items-center justify-center h-full text-center text-slate-400 dark:text-slate-600">
            <span className="material-icons-round text-6xl mb-4 opacity-30">calendar_today</span>
