  ...
  return (
    <div className="flex-1 overflow-y-auto bg-background-light dark:bg-background-dark">
      <div className="max-w-4xl mx-auto px-8 py-8">
        {/* Page header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Search Journal</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Find your saved facts, words, and insights</p>
        </div>
        ...
        <div className="flex items-center gap-2 mb-6 flex-wrap">
          ...
          {selectedCategory !== 'All' && (
            <button onClick={() => setSelectedCategory('All')} className="text-primary text-sm font-medium ml-2 hover:underline">
              Clear
            </button>
          )}
        </div>
        ...
        {results.length === 0 ? (
          <div className="flex flex-col items-center py-24 text-center gap-3">
            ...
            <p className="text-slate-500 dark:text-slate-400">No entries match your search.</p>
          </div>
        ) : (
            ...
            {results.map((item) => (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold uppercase tracking-wide ${categoryColors[item.category]}`}>
                    {item.category}
                  </span>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed line-clamp-3 flex-1">{item.content}</p>
